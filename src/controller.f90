module controller_m
    ...
    type controller_type
        ...
        type(pid_type) :: p_da, q_de, r_dr, bank_p, psi_g_bank, gamma_q, V_tau, H_gamma
        integer :: num_pid
        type(connection) :: pilot_conn
        logical :: running = .false.
        logical :: set_trim_bias = .false.
        logical :: verbose = .false.
        real :: wind(3)
        logical :: ground_track_controller_found = .false.
        logical :: altitude_controller_found = .false.
    end type controller_type


contains

    subroutine controller_init(this, j_main, wind, verbose)
        implicit none
        ...
        call pid_init(this%p_da, j_temp, verbose)

        call jsonx_get(j_pid, "q->elevator", j_temp)
        call pid_init(this%q_de, j_temp, verbose)
        
        call jsonx_get(j_pid, "r->rudder", j_temp)
        call pid_init(this%r_dr, j_temp, verbose)
        
        call jsonx_get(j_pid, "bank->p", j_temp)
        call pid_init(this%bank_p, j_temp, verbose)
        
        
        call json_get(j_pid, "psi_g->bank", j_temp, found)
        if (found) then
            this%ground_track_controller_found = .true.
            call pid_init(this%psi_g_bank, j_temp, verbose)
        end if
        
        call jsonx_get(j_pid, "gamma->q", j_temp)
        call pid_init(this%gamma_q, j_temp, verbose)

        call json_get(j_pid, "altitude->gamma", j_temp, found)
        if (found) then
            this%altitude_controller_found = .true.
            call pid_init(this%H_gamma, j_temp, verbose)    
        end if
            
        call jsonx_get(j_pid, "V->throttle", j_temp)
        call pid_init(this%V_tau, j_temp, verbose)
    ...
    function controller_update(this, states, time) result(ans)
        ...
        real :: xyzdot(3), Vg(2), Vg_mag, psi_g, psi_g_sp, H, H_sp
        ...
        ! calc ground track and ground speed 
        xyzdot = quat_dependent_to_base(states(1:3), states(10:13))  + this%wind
        Vg = xyzdot(1:2)
        ...
        if (this%ground_track_controller_found .and. this%altitude_controller_found) then
            ! ground track  AND ALTITUDE controller
            pilot_command = this%pilot_conn%recv([time], time)
            psi_g_sp = pilot_command(1)*PI/180.0
            H_sp = pilot_command(2)
            V_sp = pilot_command(3)

            if ((psi_g_sp - psi_g)>PI) then
                psi_g_sp = psi_g_sp - 2.*PI
            else if ((psi_g_sp - psi_g)<-PI) then
                psi_g_sp = psi_g_sp + 2.*PI
            end if

            ! outer outer loop
            bank_sp = pid_get_command(this%psi_g_bank, psi_g_sp, psi_g, time, dynamic_pressure)
            gamma_sp = pid_get_command(this%H_gamma, H_sp, H, time, dynamic_pressure)
        ...
        else if (this%ground_track_controller_found) then
            ! ground track controller
            pilot_command = this%pilot_conn%recv([time], time)
            psi_g_sp = pilot_command(1)*PI/180.0
    ...
    subroutine pid_init(this, j_pid, verbose)
        implicit none
    ...
    function pid_get_command(this, commanded, actual, time, dynamic_pressure) result(ans)
        ...
        if (time - this%prev_time >= 1.0/this%update_rate - TOLERANCE) then 
            ...
            if ((this%prev_ans > this%limit(1)) .and. (this%prev_ans < this%limit(2))) then
                ...
                if (this%verbose) write(*,*) this%name, " PID controller saturated at ", this%prev_ans*this%display_units,", using integrator clamping"
            end if
