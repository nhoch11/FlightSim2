module hoch_m
    ...
    real, parameter :: C_I = 7.04319971369755 ! torque onversion factor for electric motors
    real, parameter :: REZ = 6356766.0 ! [m] radius of Earth used to compute geopotential altitude
    real, parameter :: RADIUS_EARTH = 6366707.01949371 ! [m] mean sea-level radius of the Earth
...
contains
    ...
    function quat_to_euler(quat) result(euler)
        ...
        if (abs(a - 0.5) < 1.e-12) then
            ...
            if (euler(3) < 0.0) euler(3) = euler(3) + 2.*PI
        end if
    ...
    subroutine psd(x, dt, psd_norm, filename, stdev)
        implicit none
        real, intent(in) :: x(:), dt
        real, intent(out), optional :: psd_norm(:,:)
        character(len=*), intent(in), optional :: filename
        real, intent(inout) :: stdev
    ...
    function calc_polynomial(coefs, var) result(ans)
        implicit none
        real, intent(in) :: coefs(:), var
        real :: ans
        integer :: i 
        ans = 0.0
        do i=1,size(coefs)
            ans = ans + coefs(i)*var**(i-1)
        end do
